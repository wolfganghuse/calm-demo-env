...
if resp.ok:
  if (json.loads(resp.content)['status'] == "SUCCEEDED"):
    print("project_uuid={}".format(json.loads(resp.content)['entity_reference_list'][0]['uuid']))
    exit(0)
# If the call failed
else:
  print "Project UUID call failed", json.dumps(json.loads(resp.content), indent=4)
  exit(1)
